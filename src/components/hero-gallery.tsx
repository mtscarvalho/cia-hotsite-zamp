    ...
    <div className="flex items-end">
      <div className="relative aspect-960/997 w-full max-w-240 origin-bottom-right md:scale-150 lg:origin-right">
        {heroImages.map((image, index) => (
          <Image key={image.src} src={image.src} alt={image.alt} fill priority={index === 0} sizes="960px" className={["object-contain transition-opacity duration-700 ease-in-out", index === activeIndex ? "opacity-100" : "opacity-0"].join(" ")} />
        ))}
      </div>
