import { cn } from "@/lib/cn";
import { extractIdFromYouTubeUrl } from "@/lib/extract-id-from-youtube-url";

export type YouTubeEmbedProps = {
  url: string;
  title: string;
  thumb?: string;
  className?: string;
};

export function YouTubeEmbed({ url, title, className, thumb }: YouTubeEmbedProps) {
  const id = extractIdFromYouTubeUrl(url);

  return (
    <iframe
      className={cn("aspect-video h-full w-full", className)}
      width="560"
      height="315"
      src={`https://www.youtube.com/embed/${id}`}
      srcDoc={`<style>*{padding:0;margin:0;overflow:hidden}html,body{height:100%}img,span{position:absolute;width:100%;top:0;bottom:0;margin:auto}span{height:clamp(2.5rem, 10vh, 20%);text-align:center;background: url(data:image/svg+xml;base64,) no-repeat center / contain}</style><a href=https://www.youtube.com/embed/${id};autoplay=1;><img src=${thumb || `https://img.youtube.com/vi/${id}/maxresdefault.jpg`} alt='${title}'><span></span></a>`}
      allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
      allowFullScreen
      title={title}
    ></iframe>
  );
}
